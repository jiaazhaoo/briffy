...
(() => {
  ...
  const entries = [
    mk(0, 9, 12, { type: 'screenshot', title: '截图 09:12', path: 'screenshots/x.png', fileUrl: '/sample.png', tags: ['Electron', '浮动窗口', '截图', 'OCR', '工作区'], text: 'DailyLogs — 记录 / 每日摘要 / 设置\n搜索标题 / 五个词 / 文字\n2026年9月3日周四 · 8', summary: '一张关于 DailyLogs 工作区界面的截图。' }),
    ...
    mk(0, 11, 25, { type: 'url', title: 'Optical character recognition - Wikipedia', url: 'https://en.wikipedia.org/wiki/Optical_character_recognition', tags: ['OCR', 'text recognition', 'Tesseract', 'scanning', 'history'], text: 'Optical character recognition or optical character reader (OCR) is the electronic or mechanical conversion of images of typed, handwritten or printed text into machine-encoded text…', summary: 'Wikipedia article explaining OCR, its history and techniques.' }),
    ...
    mk(0, 15, 2, { type: 'note', title: '明天上午十点和产品团队开会', path: 'files/n.txt', tags: ['产品团队', '开会', '预算', '路线图', '王经理'], text: '这是一个测试笔记。\n明天上午十点和产品团队开会，讨论第三季度的预算和路线图。\n记得把会议纪要发给王经理。' }),
    mk(0, 16, 30, { type: 'image', title: 'design-ref.png', path: 'files/design-ref.png', fileUrl: '/assets/icon.png', tags: ['猫', '图标', '橙色', 'logo', '圆形'], status: 'processing', progress: '识别文字中 40%' }),
    mk(-1, 9, 5, { type: 'screenshot', title: '截图 09:05', path: 'screenshots/y.png', fileUrl: '/sample.png', tags: ['Chrome', 'Google Maps', '路线', 'Reading', '导航'], text: 'Rivermead Leisure Centre · 15 min · 2.8 miles' }),
  ...
  window.ws = {
    ...
    ollamaPull: async (model) => { for (let p = 0; p <= 100; p += 20) { await sleep(150); (listeners['ws:ollama-pull-progress'] || []).forEach((cb) => cb({ model, status: 'pulling', percent: p })); } return { ok: true }; },
    ollamaInstall: async () => { for (const line of ['$ winget install --id Ollama.Ollama -e', 'Found Ollama [Ollama.Ollama] Version 0.12.1', 'Downloading https://ollama.com/download/OllamaSetup.exe', '  ██████████████  120 MB / 700 MB', 'Successfully installed']) { await sleep(400); (listeners['ws:ollama-install-progress'] || []).forEach((cb) => cb({ line })); } return { ok: true, method: 'winget', started: true }; },
    ollamaStart: async () => { await sleep(800); return { ok: true, installed: true }; },
    onOllamaInstall: on('ws:ollama-install-progress'),
    openExtensionDir: async () => {}, exportExtension: async () => 'D:\\dailylogs-extension',
    ...
    runSetup: async ({ installOllama }) => {
      ...
      const summary = ['电脑：AMD Ryzen 5 5600X · 31.9 GB 内存 · AMD Radeon RX 6700 XT', '文字识别：PP-OCRv6 small（本地引擎，不经过大模型）', '语音识别：Xenova/whisper-small（本地）']
        .concat(installOllama ? ['本地大模型：qwen3.5:9b'] : ['还没配 AI 服务，五个词先用本地关键词']);
    ...
    chooseDir: async () => 'D:\\DailyLogs',
    listDates: async () => [...new Set(entries.map((e) => e.dateKey))].sort().reverse(),
    listEntries: async ({ query = '', dates = null } = {}) => entries.filter((e) => (!dates || dates.includes(e.dateKey)) && (!query || `${e.title} ${e.tags.join(' ')} ${e.text}`.toLowerCase().includes(query.toLowerCase()))).map(pub),
    ...
    addNote: async (text) => { const e = mk(0, today.getHours(), today.getMinutes(), { type: 'note', title: text.slice(0, 40), text, status: 'processing', progress: '正在生成五个词…' }); entries.unshift(e); (listeners['ws:entry'] || []).forEach((cb) => cb({ entry: pub(e), kind: 'add' })); return pub(e); },
    capture: async () => null,
    ...
    stats: async () => ({ days: 3, entries: entries.length }),
