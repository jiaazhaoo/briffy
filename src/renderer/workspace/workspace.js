...
(() => {
  ...
  const T = {
    zh: {
      ...
      sAnthropicAuth: '登录方式', sAccountOption: '已登录的 Claude 账号（claude auth login）', sAnthropicLogin: '用浏览器登录 Claude 账号',
      ...
      accountFound: '已检测到登录配置：{profiles}', accountEnv: '已通过环境变量提供凭据', accountNotFound: '未检测到登录，点右侧按钮在终端里完成 claude auth login',
      cliMissing: '未安装 Claude Code（装好之后这里会自动认出你已经登录的账号）',
      cliMissingCmd: '没找到 claude 命令，装好 Claude Code 之后再点一次；或者自己在终端运行：{cmd}', loginStarted: '已打开终端，按提示在浏览器里登录，完成后回来点「重新检测」',
    ...
    en: {
      ...
      sAnthropicAuth: 'Sign in with', sAccountOption: 'Signed-in Claude account (claude auth login)', sAnthropicLogin: 'Sign in to Claude in the browser',
      ...
      accountFound: 'Sign-in profile found: {profiles}', accountEnv: 'Credentials provided via environment variables', accountNotFound: 'Not signed in – click the button to run claude auth login in a terminal',
      cliMissing: 'Claude Code is not installed (once it is, this picks up the account you are already signed in to)',
      cliMissingCmd: 'claude command not found – install Claude Code and click again, or run it yourself: {cmd}', loginStarted: 'A terminal was opened – finish the browser sign-in, then click "Detect again"',
