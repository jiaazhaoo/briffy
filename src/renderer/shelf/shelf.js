...
(() => {
  ...
  const ICONS = { screenshot: '📸', image: '🖼️', audio: '🎙️', pdf: '📄', text: '📝', url: '🔗', note: '🗒️', file: '📎', video: '🎬', media: '🎬' };
  ...
  function render() {
    const list = $('#list');
    if (!items.length) { list.innerHTML = `<div class="empty">${esc(strings.empty || '')}</div>`; return; }
    list.innerHTML = items.map((e, i) => {
      const thumb = e.thumb ? `<img src="${esc(e.thumb)}" alt="" />` : (ICONS[e.type] || ICONS.file);
      // Pinned records lead the shelf and carry a dot, so something kept on purpose stays findable
      // after a busy hour instead of being pushed off the end by fifteen fresh ones.
      return `<div class="card${e.pinned ? ' pinned' : ''}" style="--i:${Math.min(i, 8)}" data-id="${esc(e.id)}" draggable="${e.file ? 'true' : 'false'}" title="${esc(e.note || e.title)}">
        <span class="th">${thumb}</span>
        <span class="tx"><span class="t">${e.pinned ? '<span class="pin"></span>' : ''}${esc(e.title)}</span><span class="m">${esc(e.note || `${time(e.createdAt)} · ${e.kind}`)}</span></span>
      </div>`;
    }).join('');
  }
