...
const STT_MODELS = [
  { id: 'Xenova/whisper-tiny.en', name: 'Whisper tiny · English only (~40 MB, bundled)', english: true, bundled: true },
  { id: 'Xenova/whisper-base.en', name: 'Whisper base · English only (~80 MB)', english: true },
  { id: 'Xenova/whisper-tiny', name: 'Whisper tiny · multilingual (~40 MB, rough)' },
  { id: 'Xenova/whisper-base', name: 'Whisper base · multilingual (~80 MB)' },
  { id: 'Xenova/whisper-small', name: 'Whisper small · multilingual (~250 MB, recommended for Chinese)' },
  { id: 'Xenova/whisper-medium', name: 'Whisper medium · multilingual (~800 MB, best, slow)' },
];
...
async function transcribe(pcm, cfg, onProgress) {
  const run = async () => {
    ...
    if (englishOnly) {
      const out = await p(pcm, { chunk_length_s: 30, stride_length_s: 5, return_timestamps: false });
      touchIdle();
...
module.exports = { transcribe, prepare, dispose, bundledDir, isBundled, STT_MODELS, DEFAULT_MODEL };
