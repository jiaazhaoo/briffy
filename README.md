...
一枚常驻在屏幕右下角的回形针——**它就是 briffy 本人**，不是一张贴上去的图片：那两道弧就是 logo 本身（[assets/brand/briffy.svg](assets/brand/briffy.svg)），由 [briffy-anim.js](assets/brand/briffy-anim.js) 现画，会动的只有脸和整体的姿态。托盘菜单里它就叫「显示 briffy / 隐藏 briffy」。

它帮你把一天里看到的、听到的、收到的东西自动记进工作区：

| 动作 | 效果 |
| --- | --- |
| **单击它** | 截整个屏幕，原图保存 + OCR，同时复制到系统剪贴板 |
| **双击它** | 框选截图：屏幕冻结后拖拽选区，回车确认、Esc 取消，同样保存 + OCR + 复制；框旁边的[长截图](#长截图)让它自己滚着拼一整页 |
| **中键点击它**（或长按 0.55 秒） | 开始 / 结束录音。触控板没有中键，长按是等价操作 |
| Ctrl+Alt+A（macOS Cmd+Shift+A） | 框选截图 |
| Ctrl+Alt+S（macOS Cmd+Shift+S） | 整屏截图 |
| Ctrl+Alt+V（macOS Cmd+Shift+V） | 开始 / 结束录音 |
| 把图片 / 文件 / 链接 / 文字拖到它身上 | 存入工作区（图片会 OCR，PDF 和网页会读取内容） |
| 右键它 / 托盘图标 | 打开工作区、设置、手动生成摘要、隐藏它、退出 |
| 复制任何东西（Ctrl+C / Cmd+C） | 剪贴板里的文字、图片、文件都会实时存入工作区 |
| 浏览器里按 Alt+Shift+D | 浏览器扩展列出当前网页所有图片 / 视频 / 音频，勾选后一键存入工作区 |

存进工作区的东西会拿到一个标题和一句话摘要；一张没有文字的图片，还会由本机的图像分类器说出画面里有什么。每天到设定时间（默认 08:00）它会为昨天的记录生成一份摘要，好了之后它身上出现一个角标；它被隐藏时改用系统通知。

存进去的东西可以直接**问出来**：工作区底栏中间那条「问」用一句话提问（「上周那个 Postgres 报错是怎么回事？」），程序先把问题里的时间和词翻成一批记录，再让 AI 只读这批记录作答，答案里每句话都标着它依据的第几条。搜索框除了精确命中，还会另外给出**意思相近**的——搜「跑步」出得来那场 walking 挑战——但它们一律标着「相近」，不和精确命中混在一起。见 [问自己的记录](#问自己的记录)。

点开工作区里的一张图，它有[自己的窗口](#看图)：画笔、马赛克、裁切、翻译、置顶、复制。别处的东西也可以[接进来](#接进来)——Notion 的页面、Gmail 的邮件，或者把导出的 zip / mbox 整包拖进来。

另外两样**默认关着、要你自己打开**：[会议自己录下来](#会议自己录下来)（别的软件开麦克风时才跟着录，录完转成文字并分出说话人 1 / 2 / 3）和[路过](#路过)（今天都在看什么，写进工作区里另一个地方，不混进记录页）。

截图会同时进系统剪贴板（像微信截图那样，截完可以直接粘贴），但**不会因此被记录两次**——程序会让剪贴板监听器跳过自己刚放进去的那张图。不想进剪贴板可以在 设置 › 快捷键 里关掉。
...
3. macOS 需要在「系统设置 › 隐私与安全性」里给应用 **屏幕录制** 和 **麦克风** 权限（开发时授权对象是 Electron）。macOS 上程序以菜单栏（托盘）应用的形式运行，点击托盘图标可打开菜单。**它不会出现在全屏应用之上**——看电影或放 PPT 时整个屏幕就是内容本身，角落里立着一枚回形针是碍事的。macOS 上全屏应用有自己的 Space，一个窗口标志就够了；Windows / Linux 没有对应机制，改成看屏幕本身：有窗口全屏时任务栏会跟着消失、可用区域涨到整块屏幕，据此隐藏。
